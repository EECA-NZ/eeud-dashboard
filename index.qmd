...
eeud_colours2 = pd.read_csv('./data/eeud-colour-palette-by-dimension.csv', header=0, dtype=str).squeeze("columns").set_index(['Label', 'Dimension'])
eeud_colours2 = eeud_colours2[~eeud_colours2.index.duplicated()]
eeud_colours2.index = eeud_colours2.index.map(lambda l: str(l).strip() if 'Unknown' not in l else l)
...
def create_sankey_links_for_dict(df, *columns):
    ...
    for i in range(len(columns) - 1):
        ...
        # Use i to differentiate levels in multi-level Sankey diagrams
        src_col = f'{columns[i]}_src_{i}'
        tgt_col = f'{columns[i+1]}_tgt_{i+1}'

        # Create modified columns in the DataFrame
        df[src_col] = df[columns[i]].astype(str) + f'_src_{i}'
        df[tgt_col] = df[columns[i+1]].astype(str) + f'_tgt_{i+1}'

        # Group by these modified columns and calculate sum
        temp_df = df.groupby([src_col, tgt_col])['TJ'].sum().reset_index()
        temp_df.rename(columns={src_col: 'source', tgt_col: 'target'}, inplace=True)
    ...
    node_labels = [l.rsplit('_', 2)[0] for l in labels]  # Remove the suffix while preserving the original label
    colormap = lambda l: hex_to_rgba(eeud_colours[l] if l in eeud_colours else "#9CA2B0", 0.8)
    source = links_df['source'].tolist()
    target = links_df['target'].tolist()
    value = links_df['TJ'].tolist()

    # Applying the colormap to the sources for link colors
    link_color = [colormap(node_labels[s]) for s in source]

    # Applying the colormap to all node labels for node colors
    node_color = [colormap(l) for l in node_labels]
