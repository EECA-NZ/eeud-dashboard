...
eeud['Fuel'] = eeud['Fuel'].replace('Electricity', 'Electricity​')
eeud['Sector'] = eeud['Sector'].replace('Residential', 'Residential​')
eeud['EndUse'] = eeud['EndUse'].replace('Irrigation', 'Irrigation​')
eeud['EndUse'] = eeud['EndUse'].replace('Refiners', 'Refiners​')
...
def create_sankey_links_for_dict(df, *columns):
    ...
    link_dict = {
        ...
        'color': links_df['source'].apply(lambda s: hex_to_rgba(eeud_colours[labels[s].split('_')[0]] if labels[s].split('_')[0] in eeud_colours else "#000000", 0.8)).tolist()
    }
...
```{python}
ui.input_select("highlight_category", "Highlight", choices=["End Use", "Fuel", "Sector", "Technology"], selected="End Use")
```
</div>
...
* Released on the 13th of May 2024.
