...
eeud_colours = pd.read_csv('./data/eeud-colour-palette-2026.csv', header=0, dtype=str).squeeze("columns").set_index(['Label', 'Dimension'])
...
WWW_IMAGES_DIR = Path("www/images")
CHART_TYPE_SVGS = {
    "bar": (WWW_IMAGES_DIR / "chart-bar-custom.svg").read_text(encoding="utf-8"),
    "sankey": (WWW_IMAGES_DIR / "chart-sankey-custom.svg").read_text(encoding="utf-8"),
    "area": (WWW_IMAGES_DIR / "chart-area-custom.svg").read_text(encoding="utf-8"),
    "line": (WWW_IMAGES_DIR / "chart-line-custom.svg").read_text(encoding="utf-8"),
}
...
def chart_type_button_label(icon_name, text):
    return ui.span(
        ui.span(ui.HTML(CHART_TYPE_SVGS[icon_name]), class_="chart-type-button-icon", **{"aria-hidden": "true"}),
...
def apply_numeric_xaxis_range_padding(fig, values, padding=0.2):
    numeric_values = pd.to_numeric(pd.Series(values).dropna(), errors="coerce").dropna()

    if numeric_values.empty:
        return fig

    min_x = numeric_values.min()
    max_x = numeric_values.max()

    fig.update_xaxes(range=[min_x - padding, max_x + padding])
    return fig

def apply_numeric_yaxis_range_padding(fig, values, padding_fraction=0.02):
    numeric_values = pd.to_numeric(pd.Series(values).dropna(), errors="coerce").dropna()

    if numeric_values.empty:
        return fig

    min_y = numeric_values.min()
    max_y = numeric_values.max()
    span = max_y - min_y
    padding = span * padding_fraction if span > 0 else max(abs(max_y) * padding_fraction, 1)
...
def display_trend():
    ...
    latest_period = merged['Period'].max()
    stack_order = (merged[merged['Period'] == latest_period]
        .sort_values('TJ', ascending=False)[group].tolist())

    fig = px.area(merged,
        y='TJ', x='Period', color=group, color_discrete_map=color_dict_for(group),
        category_orders={group: stack_order}, markers=True, 
        hover_data={group: True, 'TJ_Proportion': True}).update_xaxes(dtick=1)
...
def render_timeseries_area_button():
    ...
    return ui.input_action_button(
        "timeseries_area",
        chart_type_button_label("area", "Area"),
        class_=btn_class
    )
...
def render_timeseries_line_button():
    ...
    return ui.input_action_button(
        "timeseries_line",
        chart_type_button_label("line", "Line"),
        class_=btn_class
    )
...
def display_timeseries_chart():
    ...
    if timeseries_chart_type.get() == 'Alluvial':
        ...
        fig = apply_numeric_xaxis_range_padding(fig, timeseries_df['Period'])
        fig = apply_numeric_yaxis_range_padding(fig, timeseries_df['TJ'])
        fig = apply_plotly_gridlines(fig, yaxis=True)
        ...
        latest_period = timeseries_df['Period'].max()
        latest_order = (timeseries_df[timeseries_df['Period'] == latest_period]
            .sort_values('TJ', ascending=False)[category]
            .tolist()
            )
        fig = px.area(timeseries_df,
            y='TJ', x='Period', color=category,
            color_discrete_map=color_dict_for(category),
            category_orders={category: latest_order},
            markers=True, hover_data={category: True}).update_xaxes(dtick=1)
        fig.update_layout(yaxis=dict(title=f"{input.timeseries_display()} Energy Usage ({'TJ' if total else '%'})"))
        fig.update_layout(legend=dict(
            orientation="h",
