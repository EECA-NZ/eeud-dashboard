...
def create_sankey_links_for_dict(df, *columns):
    ...
    # Remove suffixes and reconstruct labels for display
    nodes_dict = {
        'label': [l.rsplit('_', 2)[0] for l in labels],  # Remove the suffix while preserving the original label
        'color': [hex_to_rgba(eeud_colours[l.rsplit('_', 2)[0]] if l.rsplit('_', 2)[0] in eeud_colours else "#9CA2B0", 1) for l in labels]
    }
