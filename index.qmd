...
eeud_colours = pd.read_csv('./data/eeud-2-colour-palette.csv', index_col=0, header=0, dtype=str).squeeze("columns")
eeud_colours = eeud_colours[~eeud_colours.index.duplicated()]

#eeud_colours = eeud_colours.apply(lambda x: f"#{x}" if '#' not in x else x)
...
```{python}
ui.input_select("highlight_category", "Highlight", choices=["End Use", "Fuel", "Sector", "Technology"], selected="Fuel")
```

<p style="align-content: flex-end; font-size: 1rem; padding-top: 1rem; line-height: 0px;">Values presented for 2022</p>
...
@render_widget
def display_overview_energy_use():

    category = title_case(input.highlight_category())
    group = category + "Group"
    eeud_subset = eeud[~pd.isna(eeud[category])]
    ...
    fig = px.sunburst(
        ...
        values='TJ',
        color=group,
        color_discrete_map=eeud_colours.to_dict()
    )

    fig.update_traces(marker=dict(line=dict(color='rgba(232,236,238,255)', width=1)))  # Adjust 'width' as needed for visibility
    
    #fig.update_traces(
    #    marker_colors=[eeud_colours[c] for c in pd.factorize(fig.data[0].labels)[0]],
    #    leaf_opacity=1,
    #)
...
```{python}
#| content: valuebox
#| title: "Total Energy Use 2022 (TJ)"

dict(
    icon = "lightning-charge",
    color = "secondary",
    value = f"{round(eeud[eeud.Period==max(eeud.Period)]['TJ'].sum()/1000)*1000:,}"
)
```
...
def largest_use():
    ...
    data_last_year = eeud[eeud['Period'].isin([max_year])]
...
#category = 'Fuel' #title_case(input.highlight_category())
#dict(
#    title = "Test",
#    icon = "star",
#    color = "secondary",
#    value = eeud.groupby(category)['TJ'].sum().idxmax()
#)
```
...
def largest_increase():
    category = title_case(input.highlight_category())
    max_year = eeud['Period'].max()
    data_last_two_years = eeud[eeud['Period'].isin([max_year, max_year - 1])]
    ...
    return ui.value_box(
        max_growth if len(max_growth) <=32 else max_growth[:32]+"...",
...
def largest_decrease():
    ...
    return ui.value_box(
        min_growth if len(min_growth) <=32 else min_growth[:32]+"...",
...
@render_widget
def display_overview_energy_trend():
    ...
    merged['TJ_Proportion'] = merged['TJ'] / merged['TotalTJ']
    ...
    # Create an area chart for the total TJ on the primary y-axis
    #fig = px.area(
    #    total_tj_by_period,
    #    x='Period',
    #    y='TotalTJ',
    #    labels={'TotalTJ': 'Total Energy Usage'}
    #)

    #fig.data[0].fillcolor = 'rgba(22, 64, 87, 0.1)'
    #fig.data[0].line.color = 'rgba(22, 64, 87, 0.0)'

    # Define a secondary y-axis for the proportional data
    #fig.update_layout(
    #    yaxis=dict(
    #        title='Total Energy Usage (Area)',
    #        range=[math.ceil((min(total_tj_by_period['TotalTJ']) / 1.5)/50000.0)*50000.0, math.ceil((max(total_tj_by_period#['TotalTJ']))/50000.0)*50000.0]
    #    ),
    #    yaxis2=dict(
    #        title='Proportional Energy Usage (Lines)',
    #        overlaying='y',
    #        side='right',
    #        showgrid=False  # Optional: Turn off grid lines for secondary y-axis
    #    )
    #)

    # Step 7: Add line plots of the proportions using the secondary y-axis
    #for name, group in merged.groupby(group):
    #    # Add glow effect (white line behind the main line)
    #    fig.add_trace(
    #        go.Scatter(
    #            x=group['Period'],
    #            y=group['TJ_Proportion'],
    #            mode='lines+markers',
    #            line=dict(color='#E8ECEE', width=6),  # White line wider than the main line
    #            yaxis='y2',
    ...
    #            mode='lines+markers',
    #            name=name,
    #            line=dict(color=eeud_colours.get(name, '#000000'), width=2),  # Main line color and width
    #            yaxis='y2'
    #        )
    #    )
...
```{python}
ui.input_select("treemap_category", "Highlight", choices=["End Use", "Fuel", "Sector", "Technology"], selected="Sector")
```
...
@render_widget
def display_old_category_trend():

    category = title_case(input.treemap_category())
    group = category + "Group"
...
def display_treemap():
    ...
    # Aggregate EnergyValue by Group and Category.
    category = title_case(input.treemap_category())
    group = category + "Group"
    #category, group = 'Sector', 'SectorGroup'
    eeud_aggregated = eeud.groupby([group, category], as_index=False).agg({'TJ': 'sum'})
    ...
    groups_df = pd.DataFrame({
        ...
        category: [''] * len(eeud_aggregated[group].unique()),  # Placeholder
        'EnergyValue': [0] * len(eeud_aggregated[group].unique()),  # Placeholder, value not used
        'id': eeud_aggregated[group].unique(),
        'parent': [category] * len(eeud_aggregated[group].unique())
    })
    ...
    complete_df['styled_label'] = complete_df['label'].apply(lambda x: f"{x}")
    complete_df['colour'] = complete_df[group].map(eeud_colours).fillna('#FFFFFF')

    # Plotting the treemap without the 'Root' node.
    fig = go.Figure(data=[go.Treemap(
        ids=complete_df['id'],
        labels=complete_df['styled_label'],
        parents=complete_df['parent'],
        values=complete_df['TJ'].fillna(0).astype('int'),
        marker=dict(colors=complete_df['colour']),  # Apply the colors here
        texttemplate="<span style='font-size:24px;'>%{value}</span><br><br>%{label}",
...
ui.input_select("chronistic_display", "Values", choices=["Total", "Proportional"], selected="Total")
```
...
```{python}
@render.download(
    filename=lambda: f"chronistic-{date.today().isoformat()}-{np.random.randint(100, 999)}.csv"
)
async def download_chronistic_filtered_data():
    data = chronistic_eeud_filtered()  # Ensure that this fetch is thread-safe or handled correctly within the async context
    csv_data = data.to_csv(index=False)
    await asyncio.sleep(0.25)  # Simulating some processing time
    yield csv_data
```
...
def display_chronistic_chart():
    ...
    if input.chronistic_display() == "Proportional":
        total_tj_by_period = chronistic_df.groupby('Period')['TJ'].sum().reset_index(name='TotalTJ')
        chronistic_df = pd.merge(chronistic_df, total_tj_by_period, on='Period')
        chronistic_df['TJ_Proportion'] = chronistic_df['TJ'] / chronistic_df['TotalTJ']
    ...
    if chronistic_chart_type.get() == 'Alluvial':
        ...
        fig = px.line(chronistic_df,
            ...
            markers=True).update_xaxes(dtick=1)
        fig.update_layout(yaxis=dict(title=f"{input.chronistic_display()} Energy Usage (TJ)"))
        fig.update_layout(legend=dict(
            orientation="h",
            yanchor="bottom", y=1.02,
            xanchor="left", x=0))
...
To learn more about the methodology check out the document *[Energy End Use Database (EEUD) - Key insights and methodology](#about)*.

The full dataset is available here: *[EEUD Data 2017-2024 April 2024](#about)*.
...
* Released on the ?rd of May 2024.
...
Keep up to date on new data releases through the EECA Newsletter ([sign up here](#about)) and check the website frequently for updates.
