...
def create_sankey_links_for_dict(df, *columns):
    ...
    for i in range(len(columns) - 1):
        ...
        src = f'{columns[i]}'
        tgt = f'{columns[i+1]}'
        src_col = f'{src}_src_{i}'
        tgt_col = f'{tgt}_tgt_{i+1}'
        ...
        for lbl in temp_df['source']:
            col = colormap(lbl.rsplit('_', 2)[0], src)
            label_colors.append((lbl, col))
        for lbl in temp_df['target']:
            col = colormap(lbl.rsplit('_', 2)[0], tgt)
            label_colors.append((lbl, col))

    # Remove duplicate labels and sort
    label_colors = sorted(set(label_colors))
    node_labels = [x[0] for x in label_colors]
    node_colors = [x[1] for x in label_colors]
    ...
    links_df['source'] = links_df['source'].apply(lambda x: node_labels.index(x))
    links_df['target'] = links_df['target'].apply(lambda x: node_labels.index(x))
    ...
    src = links_df['source'].tolist()
    tgt = links_df['target'].tolist()
    val = links_df['TJ'].tolist()
    link_colors = [hex_to_rgba(node_colors[s], 0.8) for s in src]
    node_colors = [hex_to_rgba(c, 1.0) for c in node_colors]

    # Remove the suffix which was used to track levels
    node_labels = [x.rsplit('_', 2)[0] for x in node_labels]
