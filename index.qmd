...
def display_overview_energy_use():
    ...
    fig.update_traces(marker=dict(line=dict(color='rgba(232,236,238,255)', width=1)))
...
def display_timeseries_chart():
    ...
    if input.timeseries_display() == "Proportional":
        ...
        timeseries_df['TJ_Proportion'] = timeseries_df['TJ'] / timeseries_df['TotalTJ']
    ...
    if timeseries_chart_type.get() == 'Alluvial':
        ...
        fig = px.line(timeseries_df,
            ...
            markers=True).update_xaxes(dtick=1)
        fig.update_layout(yaxis=dict(title=f"{input.timeseries_display()} Energy Usage (TJ)"))
        fig.update_layout(legend=dict(
            orientation="h",
            yanchor="bottom", y=1.02,
            xanchor="left", x=0))
