...
eeud.fillna({'EndUse': 'Unknown​', 'Fuel': 'Unknown​​', 'Sector': 'Unknown​​​', 'Technology': 'Unknown​​​​',
             'EndUseGroup': 'Unknown​​​​​', 'FuelGroup': 'Unknown​​​​​​', 'SectorGroup': 'Unknown​​​​​​​', 'TechnologyGroup': 'Unknown​​​​​​​​'}, inplace=True)
...
def create_sankey_links_for_dict(df, *columns):
    ...
    link_dict = {
        ...
        'color': links_df['source'].apply(lambda s: hex_to_rgba(eeud_colours[labels[s].split('_')[0]] if labels[s].split('_')[0] in eeud_colours else "#9CA2B0", 0.8)).tolist()
    }

    nodes_dict = {
        'label': labels,
        'color': list(map(lambda l: hex_to_rgba(eeud_colours[l.split('_')[0]] if l.split('_')[0] in eeud_colours else "#9CA2B0", 1), labels))
    }
...
def largest_increase():
    ...
    return ui.value_box(
        ...
        f"Largest Increase\n{max(eeud.Period)-1} to {max(eeud.Period)}",
        showcase=ui.HTML('''
...
def display_treemap():
    ...
    category = title_case(input.highlight_category())
    group = category + "Group"
    eeud_subset = eeud[(~pd.isna(eeud[category]))].copy() #&(~eeud[category].str.contains('Unknown', case=True))
    to_replace = ['Bus', 'Freight Rail', 'Heavy Truck', 'Light Commercial Vehicle', 'Light Passenger Vehicle', 'Medium Truck', 'Motorcycle', 'Passenger Rail', 'Plane', 'Ship', 'Very Heavy Truck']
    eeud_subset['TechnologyGroup'] = eeud_subset['TechnologyGroup'].apply(lambda g: 'Transport' if g in to_replace else g)
    ...
    #fig.update_traces(hovertemplate="label=%{label}<br>value=%{value}<extra></extra>")
...
def display_timeseries_chart():
    ...
    # sorted_category = timeseries_df.groupby(category)['TJ'].sum().sort_values(ascending=True).index
    # timeseries_df = timeseries_df.set_index(category).loc[sorted_category].reset_index(drop=False)
